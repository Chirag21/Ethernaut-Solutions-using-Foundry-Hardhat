...
### _Never place private keys, secret phrases, or mnemonics in a .env file associated with real funds. Only put private keys in a .env file with ONLY testnet ETH, LINK, or other cryptocurrencies._
...
### Getting Started
The repo includes Ethernaut solutions built with Hardhat and Foundry. The `src/` folder contains the Ethernaut-level contracts. The tests for the exploits are present in the `test/` folder. `script` and `scripts` folders contain the scripts that can be run on the testnet. These scripts carry out the exploitation.
...
### Spin up local node
Get private RPC url from [Alchemy](https://www.alchemy.com/overviews/private-rpc-endpoint). This allows transactions to be broadcasted more quickly than the public RPC url. 
...
- Once the simulation is successful, run the test on the testnet network. After successful execution, go to the level page on Ethernaut and submit the instance. __Remember to use `"PRIVATE_KEY"` from `.env`__.
